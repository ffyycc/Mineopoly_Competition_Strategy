...
public class MineopolyMain {
    ...
    public static void main(String[] args) {
        if (TEST_STRATEGY_WIN_PERCENT) {
            MinePlayerStrategy yourStrategy = new AggressiveStrategy(); // TODO: Replace this with your strategy
            MinePlayerStrategy enemyStrategy = new RandomStrategy();
            int[] assignmentBoardSizes = new int[]{14,20,26,32};

            for (int testBoardSize : assignmentBoardSizes) {
                double strategyWinPercent = getStrategyWinPercent(yourStrategy, enemyStrategy, testBoardSize);
    ...
    private static void playGameOrReplay() {
        ...
        if (savedReplayFilePath == null) {
            ...
            MinePlayerStrategy redStrategy = new AggressiveStrategy(); // TODO: Replace this with your strategy
    ...
    private static double getStrategyWinPercent(MinePlayerStrategy yourStrategy,MinePlayerStrategy enemyStrategy , int boardSize) {
        ...
        for (int i = 0; i < numTotalRounds; i++) {
            GameEngine newGame = new GameEngine(boardSize, yourStrategy, enemyStrategy, System.currentTimeMillis());
            newGame.runGame();
            if (newGame.getRedPlayerScore() >= newGame.getMinScoreToWin()) {
